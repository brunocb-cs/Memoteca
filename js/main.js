...
async function adicionarChaveAoPensamento(){
  try {
    ...
    alert("Erro ao adicionar chave do pensamentos")
  }
}
...
const regexConteudo = /^[A-Za-z.!?á-ó\s]{10,}$/
...
async function manipularSubmissaoFormulario(event) {
  ...
  if (!validarConteudo(conteudoSemEspacos)) {
    alert("É permitida no conteúdo a inclusão apenas de letras e espaços, com no mínimo 10 caracteres")
    return
  }

  if (!validarAutoria(autoriaSemEspacos)) {
    alert("É permitida na autoria a inclusão apenas de letras e espaços, com no mínimo 3 a 15 caracteres")
    return
  }
